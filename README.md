...
**Nothing ships unverified.** No agent grades its own work. Every meaningful change goes to a blind, fresh-context reviewer that gets your original request verbatim, none of the worker's reasoning, and a named set of checks. That reviewer's verdict supplies *evidence*, never acceptance — a reviewer verdict alone is never acceptance proof. The lead accepts: it reads the actual candidate, reconciles your original requirements against the evidence, adjudicates the consequential findings, and personally checks critical behavior, material gaps, and disputed claims; a required observation that could not be made is reported incomplete rather than passed. Any family — Claude, Codex, or Grok — can hold a reviewer seat when it is qualified for that seat (model, transport, allowed tools, and the specific checks assigned), and a reviewer that cannot run a check reasons independently over the candidate while a named executor runs the check. Which model actually served a dispatch is established from evidence, never from the model's own say-so, and every dispatch, verdict, and cost lands in an append-only ledger.
...
No dated model IDs in routing policy. No configuration files. One skill, five agent roles, five small deterministic scripts, and a set of rules good enough that a frontier model actually follows them.

## What's new in v0.5.0 — the lead accepts, and the crew keeps a record

1. **Acceptance belongs to the lead.** A reviewer verdict supplies evidence; it is never acceptance, and a reviewer verdict alone is never acceptance proof. The lead inspects the actual candidate, reconciles the original requirements against the evidence, adjudicates the consequential findings, and personally checks critical behavior, material gaps, and disputed claims. A required observation that cannot be made is reported incomplete, not passed.
2. **Reviewer qualification replaces family eligibility.** All three families — Claude, Codex, Grok — are eligible for any role, including the reviewer seat, when qualified: model + transport + allowed tools + the specific checks assigned. Every required deterministic observation names its executor; a no-shell reviewer does the independent reasoning while the lead or a designated capable executor runs the candidate-bound checks, and an unrun check stays unverified. The Claude reviewer remains the *default* evidence seat because its no-edit tool contract plus the mutation backstop gives **detection** of tracked changes — contract-plus-detection, not a sandbox. Seat-evidence tier is disclosed in the ledger, never used to bar a seat from a role.
3. **Effort is chosen per dispatch by the lead.** No table prescribes a reasoning-effort level any more. For Grok the no-evidence prior is the highest supported level, revised as the performance record accumulates; effort discretion exists only where the model and transport support it.
4. **A behavior test decides whether a change needs review.** The old "single-file, zero-logic" exemption is gone: a change that cannot alter behavior (comments, formatting, prose) may skip independent review; anything that can alter behavior may not, however small the diff.
5. **Recovery is outcome-bound, and a run parks instead of halting.** After two failed fix waves on one outcome the lead writes one reservation and runs exactly one diagnosed, changed recovery. If that fails, the outcome is parked as `NEEDS USER` with evidence, the missing criterion, a concrete question, and a resume condition; dependents are parked with their links, independent authorized work continues, and the goal is never reported complete while an original outcome is parked. A rename, split, re-seat, or restart never resets a count.
6. **A global performance record.** With your authorization, each closed outcome appends one record to `~/.foreman/crew-performance.md` via `scripts/crew-append.sh` — serialized, append-only, idempotent on run + outcome + closure revision, corrections appended rather than rewritten, with a local receipt when the append fails. The lead reads the relevant slice before a comparable dispatch. No cross-machine sync is assumed. It records what happened and refuses to conclude universal provider rankings, accuracy without a denominator, or savings from one uncontrolled campaign.
7. **The run ledger gains `## Current`, reservations, and a per-run crew record** — parked questions and held write surfaces overwritten in place; a reservation line written before every dispatch with the seat identity appended after; a `LAUNCH UNKNOWN` reservation reconciled against live processes before anything is re-dispatched.
8. **Honest limits, stated as limits.** Sequential dispatch is preferred for likely cache reuse, but cache reuse is not guaranteed — the skill records cache telemetry when the provider exposes it instead of claiming a saving. Fifteen concurrent workers is a *reported* default ceiling, not a measured maximum. Provider telemetry lands as `unavailable` when the raw value is missing, and a launcher `CONTEXT ALERT` is an average-based routing warning, never proof that any individual call was repriced.

**Known follow-up, not in this release:** the `scripts/grok-dispatch.sh` `CONTEXT ALERT` message still says "every call was repriced" where it means "the average exceeded 200K; individual calls may or may not have been repriced." The launchers are unchanged in v0.5.0 and the wording is a separate ticket.

## What's new in v0.4.0 — know what a seat costs  *(historical: released 2026-08-18; the policy items below marked "historical" were superseded in v0.5.0)*
...
4. **Self-correction is automatic, and bounded.** Confirmed findings go to a fix worker and then to a *fresh* reviewer; a reviewer judges the candidate and does not change it within that same assignment. (v0.5 makes the reverse path explicit rather than forbidden: the foreman may re-dispatch that reviewer as a writable fixer under a new journaled ticket, the earlier verdict does not cover the new edits, and a different fresh context assesses the repaired revision.) It still stops — for an ask that was advisory in the first place (a review is not a licence to implement), for a design or user-visible-contract decision the user owns, for external blockers, destructive actions, policy refusals, or a bar no qualified reviewer clears. v0.5 bounds it further: two failed fix waves on one outcome buy exactly one diagnosed, changed recovery, and a failed recovery parks the outcome as `NEEDS USER` while independent work continues.
5. **Route to what is actually there.** Harness capabilities and the provider pool are two independent axes now, not an enumerated combination table. Claude-only, Claude+Codex, Claude+Grok, or all three — all legal. The foreman never stalls on an absent provider and never asks you to install one mid-run. *(Historical, v0.4: in every combination the accepting verdict is the Claude verifier. v0.5 replaced that rule — the lead accepts, and a qualified reviewer of any family supplies the evidence.)*
6. **A `BILLED` evidence tier, ranked below `SERVED`.** Grok's envelope names the model that was *charged* for the turn — real evidence, better than a self-report, but a billing SKU is not the weights that answered, so it never raises a seat to `verified`. In v0.5 that tier is a **disclosure**, not a disqualification: it is recorded in the ledger and weighed by the lead, and it no longer bars a seat from a role. *(Historical, v0.4: a billed-tier Grok seat advises and never accepts.)*
...
**Deliberately not changed:** the First Law is untouched — an earlier draft would have made it "pool-aware" so quota pressure could move the quality bar, and that was dropped. The ledger discipline is the same. *(Historical, v0.4: Grok cannot hold a verifier verdict. v0.5 removed the family bar; a Grok seat is qualified or not by model, transport, tools, and assigned checks, like any other.)*
...
| Grok 4.5 | 2 | 6 | 85% off | 500K | 56 | historical, dated 2026-08-17: no `xhigh` support; sending it errors |
...
|---|---|---|
| Architecture, ambiguous debugging, final judgment | LEAD (whichever frontier-class model holds the session) | luna, Haiku |
| Review evidence for an acceptance decision (blind reviewer) | **Claude reviewer — the default evidence seat**; the lead accepts | a reviewer verdict alone as acceptance proof |
| Adversarial review / second opinion | **Grok 4.6**, effort by lead judgment, then Codex sol | — |
| Well-specified implementation, tests, refactors | **Grok 4.6**, effort by lead judgment (<200K) when cost dominates, else terra | — |
| Large-context implementation (>200K) | **Sonnet 5**, then terra | **Grok (cliff)**, Haiku (200K cap) |
| Mechanical edits, extraction, scanning | **gpt-5.6-luna**, effort by lead judgment, then Haiku 4.5 | frontier seats |
...
4. **Collects four-status reports** — `DONE / DONE_WITH_CONCERNS / NEEDS_CONTEXT / BLOCKED` — with a bounded escalation ladder: two failures at a seat, then escalate one seat or take over. Never a third identical retry. After two failed fix waves on one outcome the lead runs exactly one diagnosed, changed recovery; if that fails, the outcome is parked as `NEEDS USER` with evidence and a concrete question, and the run continues on independent work instead of halting.
5. **Verifies like it trusts no one** — the project's real build/test command first (free), then a blind fresh-context reviewer that gets the original task verbatim and none of the worker's reasoning. The trigger is a behavior test, not a file count: a change that cannot alter behavior may skip independent review, and anything that can alter behavior may not. Reviewers of any family sharpen the read — a Codex read-only reviewer is the strongest cross-family second opinion, Grok the cheap adversarial one — and their verdicts supply evidence and challenge, not acceptance: the lead accepts, inspecting the candidate itself and personally checking critical behavior, material gaps, and disputed claims. When a reviewer resolves to the same model as the lead, it says so — "blind-verified (same model, independent context)" — instead of implying independence it didn't obtain.
6. **Respects your budget both directions** — sequential dispatch by default (cache reuse is likelier that way, but it is not guaranteed; the skill records cache telemetry when a provider exposes it rather than claiming a saving), announced fan-outs bounded by a *reported* default ceiling of fifteen concurrent workers — a reported figure, not a measured maximum — and the degradation rule: under quota pressure it steps seats down *visibly* and prefers stopping cleanly over silently shipping degraded work. **Economics never lowers the quality bar.**
7. **Keeps a performance record across runs** — with your authorization, each closed outcome appends one record to `~/.foreman/crew-performance.md` through `scripts/crew-append.sh` (serialized, append-only, corrections appended rather than rewritten, a local receipt when the append fails), and the lead reads the relevant slice before a comparable dispatch. It records outcomes; it refuses to conclude universal provider rankings, accuracy without a denominator, served identity from self-report, or savings from one uncontrolled campaign.
